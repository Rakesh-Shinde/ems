...
function EditEmployee() {
	...
	useEffect(() => {
		axios.get('http://localhost:8081/get/' + id, { headers: { "token": localStorage.getItem("token") } })
			.then(res => {
				setData({
	...
	const handleSubmit = (event) => {
		...
		axios.put('http://localhost:8081/update/' + id, data, { headers: { "token": localStorage.getItem("token") } })
			.then(res => {
